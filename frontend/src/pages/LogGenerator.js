...
import { Card, Button, Form, Row, Col, ProgressBar, Badge, ListGroup, Container, Tabs, Tab } from 'react-bootstrap';

const inferLogLevelClient = (message) => {
if (!message || !message.trim()) return 'WARN'; // [수정] 기본값을 WARN으로 변경 (INFO가 서버에 없으므로)
...
setFormData({ ...formData, ...presets[presetKey] });
};
                ...
                <Card.Body>
                    <Form onSubmit={handleBasicSubmit}>
                        ...
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control as="textarea" rows={4} value={formData.stackTrace} onChange={(e) => setFormData({...formData, stackTrace: e.target.value})} className="font-monospace small bg-light" />
                        </Form.Group>
                        <hr />
                        ...
                        <Form.Control type="number" min="0" value={delayMs} onChange={(e) => setDelayMs(Number(e.target.value))} />
                        </div>
                        ...
                        <Card.Body>
                            ...
                            <ListGroup variant="flush" className="small mb-3">
                                ...
                                <ListGroup.Item key={idx} className="px-0 py-1 border-0">
                                    <Badge bg={step.logLevel === 'ERROR' || step.logLevel === 'FATAL' ? 'danger' : 'secondary'} className="me-2">
                                    {step.logLevel}
                                    </Badge>
                                    x{step.count || 1}
                                </ListGroup.Item>
