...
5. [AdvancedResilienceTest](#5-advancedresiliencetest)
6. [성능 SLO 기준](#6-성능-slo-기준)
7. [실행 방법](#7-실행-방법)
...
| 테스트 파일 수 | 3개 |
| 총 테스트 케이스 | 36개 |
| 테스트 분류 | 정규화 · 기본동작 · 성능 · KB연동 · LogFixer연동 · 내결함성 · 동시성 · Redis Fallback · 신뢰도 임계치 |
| 성능 SLO 적용 | 4개 항목 |
...
└── AdvancedResilienceTest.java       #  7개 (동시성 · Redis Fallback · 신뢰도 임계치)
```
...
## 5. AdvancedResilienceTest

> 파일: `AdvancedResilienceTest.java` | 총 **7개** 테스트
> `@AutoConfigureMockMvc` 포함 — 신뢰도 임계치 테스트에서 MockMvc 사용

### Block 1 — 동시성 (2개)

동일 logHash로 20개 스레드가 `saveLog()`를 동시에 호출하는 race condition 시나리오.

| ID | 테스트명 | 검증 내용 |
|---|---|---|
| T1 | 동일 logHash 20건 동시 요청 | `errorLogRepository.count() == 1`, `incidentRepository.count() == 1` |
| T2 | 20건 완료 후 repeatCount | `repeatCount == 20` (DB 레벨 atomic upsert로 카운트 누락 없음) |

**동시성 제어 메커니즘:**
- LC: `errorLogRepository.upsertErrorLog()` → MariaDB `ON DUPLICATE KEY UPDATE repeat_count + 1` (원자적)
- KB: `incidentRepository.upsertIncident()` → `unique(log_hash)` + `IncidentBridgeService` manual retry (최대 3회)
- LC 서비스: `@Retryable(ConcurrencyFailureException, maxAttempts=3)`

### Block 2 — LogFixer 신뢰도 임계치 (3개)

`POST /api/kb/{kbArticleId}/addendums` 요청 시 `confidence` 필드 검증.

**구현 위치:**
- `KbAddendumCreateRequest.confidence` (Double, optional)
- `KbAddendumService.MIN_CONFIDENCE = 0.4`
- confidence < 0.4 → `IllegalArgumentException` → `GlobalExceptionHandler` → **400 Bad Request**

| ID | confidence 값 | 기대 응답 | 저장 여부 |
|---|---|---|---|
| T3 | `0.35` (< 0.4) | **400** Bad Request | 저장 안됨 |
| T4 | `0.6` (≥ 0.4) | **200** OK | 저장됨 |
| T5 | 미제공 (null) | **200** OK | 저장됨 (기존 호환성) |

### Block 3 — Redis Fallback (2개)

Redis 컨테이너를 **강제 종료**해 실제 장애 상황을 재현.
`LogToRedis.push()` → `RedisConnectionFailureException` 감지 → `safeFallbackToDb()` → `errorLogCrdService.saveLog()` 직접 호출.

> **주의**: T6에서 Redis를 종료하므로 **반드시 마지막 순서(@Order 99, 100)** 에 실행.

| ID | 테스트명 | 검증 내용 |
|---|---|---|
| T6 | Redis 종료 후 10건 push | `errorLogRepository.count() == 10` (유실 0건) |
| T7 | Redis 장애 중 동일 로그 5건 | `count == 1`, `repeatCount == 5` (dedup 정상 동작) |
...
# AdvancedResilienceTest 단독 (동시성 + 신뢰도 + Redis Fallback)
export RUN_INTEGRATION_TEST=true
./gradlew test --tests "com.soyunju.logcollector.perf.AdvancedResilienceTest"
```
